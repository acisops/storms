...
ace_channels = ["P1", "P3", "P5", "P7", "DE1", "DE4"]
...
p_ebins = np.array([[46, 67], [112, 187], [310, 580], [1060, 1910]])
...
class SolarWind:
            ...
            with h5py.File(fn, "r") as f:
                ...
                for k in ace_channels:
                    if use_browse:
                        ...
                        v = d[k][idxs]
                    table[k].append(v)
        self.ace_table = Table(
            {k.lower(): np.concatenate(table[k]) for k in ace_channels}
        )
    ...
    def _find_txings_files(self):
        ...
        for fn in fns:
            with open(fn, "r") as f:
                ...
                tstartf, tstopf = convert_txings_date(
    ...
    def _get_txings(self):
        ...
        t = vstack([ascii.read(fn, names=txings_cols) for fn in txings_files])
    ...
    def generate_slopes(self):
        self.ace_table["ace_soft_slope"] = find_index(
            self, p_emid[:-1], ["p1", "p3", "p5"]
        )
        self.ace_table["ace_hard_slope"] = find_index(
            self, p_emid[1:], ["p3", "p5", "p7"]
        )
        self.hrc_table["goes_soft_slope"] = find_index(
    ...
    def _plot_txings(self, plot=None):
        ...
        for o in self.obsids:
            ...
            if not first:
                self.rates["times"].append(
                    [0.5 * (this_time[0] + self.rates["times"][-1][-1])]
                )
